...
if __name__ == "__main__":
    ...
    if args.datasets[0] == "default":
        ...
        args.datasets = ["cifar-10", "cifar-100", "imagenet-200", "imagenet-1k"]
    ...
    for content in args.contents:
        if content == "datasets":
            ...
            if not os.path.exists(os.path.join(store_path, "benchmark_imglist")):
                gdown.download(
                    id=download_id_dict["benchmark_imglist"], output=store_path
                )
            ...
            for checkpoint in args.checkpoints:
                if require_download(checkpoint, store_path):
                    gdown.download(id=download_id_dict[checkpoint], output=store_path)
